...
tutorial_label = tkinter.Label(text="教程:请先注册智谱AI的账号并获取密钥才能使用，根据按钮的提示进行操作", bg="red", fg="white") 
tutorial_label.place(x=50,  y=0) 
 
key_label = tkinter.Label(text="请先输入你的密钥：") 
key_label.place(x=50,  y=30) 
 
key_entry = tkinter.Entry(root, width=33) 
key_entry.place(x=50,  y=50) 
...
def on_resize(event):
    new_x = root.winfo_width()  - 750
    new_y = root.winfo_width()  - 650
...
def upload(): 
    ...
    if img_path_tuple: 
        ...
        tkinter.ttk.Button(root,  text="确定识别", command=lambda: start_classify_thread(img_path), style="TButton").place(x=180, y=80) 
        global global_photo 
        try: 
            img = Image.open(img_path)  
            photo = ImageTk.PhotoImage(img) 
            global_photo = photo  # 保存图片对象，防止被垃圾回收 
            image_label.config(image=photo)   # 更新图片显示
        except Exception as e: 
            messagebox.showerror("  错误", f"打开图片时出错: {e}") 
    else: 
        print('未选择图片') 
        messagebox.showwarning("  警告", "未选择图片") 
 
root.mainloop()  
>>>>>>> 72e5ef542bfed4c322c09a89094abf2356066844
